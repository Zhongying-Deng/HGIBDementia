...
class SingleTimeMultiModalityClassificationHGIBSSLModel(BaseModel):
    def name(self):
        return 'SingleTimeMultiModalityClassificationHGIBSSLModel'
    ...
    def forward(self, phase='train', data=None):
        if phase == 'train':
            self.set_input(data)
            self.ExtractFeatures(phase='train')
            self.HGconstruct(self.embedding_MRI, self.embedding_PET, self.embedding_NonImage)
            self.set_HGinput(self.target)
            length = [data.shape[0], 0]
            self.info(length)
