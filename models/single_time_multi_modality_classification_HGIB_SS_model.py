        ...
        if phase == 'train':
            ...
            for i in range(len_train_loader): 
                ...
                if False:
                    ...
                    #self.loss_cls += self.criterionCE(prediction_x_graph, self.target) + weight_u * ((prediction_x_graph - prediction_x)**2).sum(1).mean()
                    self.loss_cls = self.loss_cls + weight_u * ((prediction_x_graph - prediction_x)**2).sum(1).mean()
