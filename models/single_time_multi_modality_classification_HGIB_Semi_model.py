...
class SingleTimeMultiModalityClassificationHGIBSemiModel(BaseModel):
    def name(self):
        return 'SingleTimeMultiModalityClassificationHGIBSemiModel'
    ...
    def HGconstruct_unlabeled(self, embedding_MRI, embedding_PET, embedding_NonImage):
        G = Hypergraph.from_feature_kNN(embedding_MRI, self.K_neigs, self.device)
        G.add_hyperedges_from_feature_kNN(embedding_PET, self.K_neigs)
        G.add_hyperedges_from_feature_kNN(embedding_NonImage, self.K_neigs)
        self.G_unlabeled = G
        ...
        if phase == 'train':
            if self.train_encoders:
                ...
                for i in range(len_train_loader): 
                    ...
                    #if self.MRI.shape[0] < self.K_neigs:
                    ...
                    self.optimizer.step()
                    if i % 100 == 0:
                        print('Iteration {}, loss for encoders {}'.format(i, self.loss.item()))
        ...
        for i in range(num_graph_update):
            ...
            if not self.train_encoders and phase == 'train':
                if i == 0:
                    ...
                    #self.MRI_hg, self.PET_hg, self.Non_Img_hg, self.Label_hg, self.length_hg = self.get_features([train_loader_u, test_loader], update_target=False)
                    self.MRI_hg, self.PET_hg, self.Non_Img_hg, self.Label_hg, self.length_hg = self.get_features([train_loader_u, train_loader], update_target=False)
                # create hypergraph
                self.HGconstruct_unlabeled(self.MRI_hg, self.PET_hg, self.Non_Img_hg)
                #self.info(self.length_hg)
                self.set_HGinput()
                embedding_u = torch.Tensor(np.hstack((self.MRI_hg, self.PET_hg, self.Non_Img_hg))).to(self.device)
                prediction_u_graph = self.netDecoder_HGIB(embedding_u, self.G_unlabeled)
                prediction_u_graph = F.softmax(prediction_u_graph[0][-1], 1)
                # embedding = self.embedding_MRI + self.embedding_PET + self.embedding_NonImage
                prediction_u = self.netClassifier(embedding_u)
                prediction_u = F.softmax(prediction_u, 1)
    ...
    def get_features(self, loaders, update_target=True):
        ...
        for idx, loader in enumerate(loaders):
            for i, data in enumerate(tqdm(loader)):
                self.set_input(data, update_target=update_target)
